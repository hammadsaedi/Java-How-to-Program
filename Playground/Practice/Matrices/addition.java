...
// Date and Time: 18 April 2022 4:48 pm
...
public class addition {
    ...
    public static void main(String[] args){
        ...
        // dimension of matrices (Should be same to perform addition operations)
        rows = 3;
        columns = 2;
        ...
        answer = matrixAddition(matrix1, matrix2);
    ...
    // method to return additions of two matrices
    public static int[][] matrixAddition(int[][] matrix1, int[][] matrix2){
        ...
        for (i = 0; i < matrix.length; i++){ // iteration through rows
            for (j = 0; j < matrix[i].length; j++){ // iterations through column
                matrix[i][j] = matrix1[i][j] + matrix2[i][j]; // addition of consecutive element of matrix
            }
        }
